...
class MockTransport {
  ...
  async auth(route: Route, pathname: string) {
    ...
    if (method === 'POST' && pathname === '/api/auth/login') {
      ...
      await this.setSessionCookie(email)
  ...
  async todo(route: Route, pathname: string) {
    ...
    const invalid = (data: unknown = null, message = '批量请求无效') => this.send(route, 400, { code: data ? 40002 : 40001, message, data })
    const idsFrom = (value: unknown): number[] | undefined => Array.isArray(value) && value.length >= 1 && value.length <= 100 && value.every((id) => Number.isInteger(id) && id > 0) && new Set(value).size === value.length ? value as number[] : undefined
    ...
    if (method === 'POST' && pathname === '/api/todos/batch') {
      if (!Array.isArray(body.items) || body.items.length < 1 || body.items.length > 100) return invalid()
      const now = iso(); const created: Todo[] = []
      for (const [index, raw] of body.items.entries()) { const item = raw as Record<string, unknown>; if (typeof item.title !== 'string' || !item.title.trim()) return invalid({ index, id: 0, field: 'title' }, '待办标题不能为空'); if (item.priority !== undefined && !['high', 'medium', 'low'].includes(String(item.priority))) return invalid({ index, id: 0, field: 'priority' }, '优先级无效'); created.push({ id: state.nextId + index, title: item.title.trim(), description: typeof item.description === 'string' ? item.description : '', priority: item.priority === 'high' || item.priority === 'low' ? item.priority : 'medium', completed: false, due_date: typeof item.due_date === 'string' ? item.due_date : null, created_at: now, updated_at: now }) }
      state.nextId += created.length; state.todos.unshift(...[...created].reverse()); return this.send(route, 201, ok({ items: created, count: created.length }))
    }
    if (method === 'POST' && pathname === '/api/todos/batch/get') { const ids = idsFrom(body.ids); if (!ids) return invalid(); const items = ids.map((id) => state.todos.find((todo) => todo.id === id)); if (items.some((item) => !item)) return notFound(); return this.send(route, 200, ok({ items, count: items.length })) }
    if (method === 'PUT' && pathname === '/api/todos/batch') {
      if (!Array.isArray(body.items) || body.items.length < 1 || body.items.length > 100) return invalid(); const inputs = body.items as Record<string, unknown>[]; const ids = idsFrom(inputs.map((item) => item.id)); if (!ids) return invalid(); const next = state.todos.map((todo) => ({ ...todo })); const result: Todo[] = []
      for (const [index, item] of inputs.entries()) { const target = next.find((todo) => todo.id === item.id); if (!target) return notFound(); if (item.title !== undefined && (typeof item.title !== 'string' || !item.title.trim())) return invalid({ index, id: item.id, field: 'title' }, '待办标题不能为空'); if (item.priority !== undefined && !['high', 'medium', 'low'].includes(String(item.priority))) return invalid({ index, id: item.id, field: 'priority' }, '优先级无效'); Object.assign(target, { ...(typeof item.title === 'string' ? { title: item.title.trim() } : {}), ...(typeof item.description === 'string' ? { description: item.description } : {}), ...(['high', 'medium', 'low'].includes(String(item.priority)) ? { priority: item.priority } : {}), ...(typeof item.due_date === 'string' || item.due_date === null ? { due_date: item.due_date } : {}), ...(typeof item.completed === 'boolean' ? { completed: item.completed } : {}), updated_at: iso() }); result.push(target) }
      state.todos = next; return this.send(route, 200, ok({ items: result, count: result.length }))
    }
    if (method === 'PATCH' && pathname === '/api/todos/batch/status') { const ids = idsFrom(body.ids); if (!ids || typeof body.completed !== 'boolean') return invalid(); if (ids.some((id) => !state.todos.some((todo) => todo.id === id))) return notFound(); const result = ids.map((id) => ({ ...state.todos.find((todo) => todo.id === id)!, completed: body.completed as boolean, updated_at: iso() })); const changed = new Map(result.map((todo) => [todo.id, todo])); state.todos = state.todos.map((todo) => changed.get(todo.id) ?? todo); return this.send(route, 200, ok({ items: result, count: result.length })) }
    if (method === 'DELETE' && pathname === '/api/todos/batch') { const ids = idsFrom(body.ids); if (!ids) return invalid(); const items = ids.map((id) => state.todos.find((todo) => todo.id === id)); if (items.some((item) => !item)) return notFound(); const selected = new Set(ids); state.todos = state.todos.filter((todo) => !selected.has(todo.id)); return this.send(route, 200, ok({ items, count: items.length })) }
...
export async function installMockTransport(context: BrowserContext) {
  const state = new MockTransport(context)
