  ...
  return (
    <aside className="agent-panel" aria-label="智能助手面板">
      ...
      <div className="agent-panel__body" role="log" aria-live="polite" aria-label="对话消息">
        ...
        <div className="agent-panel__suggestions">{suggestions.map((suggestion) => <button type="button" key={suggestion} disabled={busy} onClick={() => session.send(suggestion)}>{suggestion}</button>)}</div>
      ...
      <footer className="agent-panel__composer">
        <div>
          <textarea value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={onKeyDown} placeholder="输入消息或指令…" aria-label="消息输入框" rows={2} disabled={busy} />
          <IconButton label="发送消息" tone="primary" icon={<SendIcon />} disabled={!draft.trim() || busy} onClick={send} />
        </div>
