...
export default function ProfilePage() {
  ...
  return (
    <section className="profile-page">
      <header className="profile-page__header"><div><p>账户中心</p><h1>个人资料</h1><span>管理登录账户与工作偏好</span></div><Button onClick={save} disabled={pending}>{pending ? '保存中...' : '保存修改'}</Button></header>
      ...
      <div className="profile-grid">
        <section className="panel-surface profile-form"><h2>账户信息</h2><TextField label="显示名称" value={name} onChange={(event) => setName(event.target.value)} /><TextField label="邮箱地址" type="email" value={email} onChange={(event) => setEmail(event.target.value)} /><TextField label="时区" value={timezone} onChange={(event) => setTimezone(event.target.value)} /></section>
