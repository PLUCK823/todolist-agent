...
def _step_from_row(row: asyncpg.Record) -> StepRecord:
    return StepRecord(
        ...
        status=row["status"],
        tool=row["tool"],
        args=_json_from_database(row["args"]) or {},
        result=_json_from_database(row["result"]),
...
class HistoryRepository:
    ...
    async def create_session(
        self, owner_id: UUID, title: str = DEFAULT_SESSION_TITLE
    ) -> SessionSummary:
    ...
    async def get_session(
        ...
        async with self._pool.acquire() as conn:
            session = await conn.fetchrow(
                """SELECT id, owner_id, title, created_at, updated_at, last_message_at
                   FROM agent_sessions WHERE id = $1 AND owner_id = $2""",
                session_id,
                owner_id,
            )
            if session is None:
            ...
            turn_rows = await conn.fetch(
                """SELECT t.id, t.session_id, t.ordinal, t.status, t.started_at, t.completed_at,
                   ...
                   WHERE t.session_id = $1 AND s.owner_id = $2 ORDER BY t.ordinal""",
                session_id,
                owner_id,
            )
            message_rows = await conn.fetch(
                """SELECT m.id, m.session_id, m.turn_id, m.role, m.content, m.ordinal, m.created_at
                   ...
                   WHERE m.session_id = $1 AND s.owner_id = $2 ORDER BY m.ordinal""",
                session_id,
                owner_id,
            )
            step_rows = await conn.fetch(
                """SELECT st.id, st.turn_id, st.event_id, st.ordinal, st.label, st.status, st.tool,
                   ...
                   WHERE t.session_id = $1 AND s.owner_id = $2 ORDER BY t.ordinal, st.ordinal""",
                session_id,
                owner_id,
            )
        ...
        turns = tuple(
            TurnRecord(
                ...
                result_uncertain=row["result_uncertain"],
                messages=tuple(messages.get(row["id"], ())),
    ...
    async def delete_session(self, owner_id: UUID, session_id: UUID) -> bool:
        async with self._pool.acquire() as conn:
            result = await conn.execute(
                "DELETE FROM agent_sessions WHERE id = $1 AND owner_id = $2",
                session_id,
                owner_id,
            )
    ...
    async def start_turn(
        ...
        async with self._pool.acquire() as conn, conn.transaction():
            ...
            existing = await conn.fetchrow(
                """SELECT t.id, t.session_id, t.ordinal, t.status, t.started_at,
                          t.completed_at, t.failure_code, t.failure_message,
                          t.result_uncertain, m.id AS message_id,
                          m.session_id AS message_session_id,
                          m.turn_id AS message_turn_id, m.role AS message_role,
                          m.content AS message_content
                   FROM agent_turns t
                   LEFT JOIN agent_messages m ON m.id = $2
            ...
            if existing is not None:
                exact_retry = (
                    ...
                    and existing["message_session_id"] == session_id
                    and existing["message_turn_id"] == turn_id
                    and existing["message_role"] == "user"
                    and existing["message_content"] == content
                )
                if not exact_retry:
                    raise HistoryConflictError("start turn identity conflict")
                return TurnRecord(
                    id=existing["id"],
                    ...
                    result_uncertain=existing["result_uncertain"],
                )
            turn_ordinal = await conn.fetchval(
                "SELECT COALESCE(MAX(ordinal), 0) + 1 FROM agent_turns WHERE session_id = $1",
                session_id,
            )
            message_ordinal = await conn.fetchval(
                "SELECT COALESCE(MAX(ordinal), 0) + 1 FROM agent_messages WHERE session_id = $1",
                session_id,
            )
            await conn.execute(
    ...
    async def upsert_step(
        self, owner_id: UUID, turn_id: UUID, event: PersistedStepEvent
    ) -> bool:
        ...
        truncated = (
            ...
            and len(result_json.encode("utf-8")) > RESULT_MAX_BYTES
        )
        ...
        async with self._pool.acquire() as conn, conn.transaction():
            owned_turn = await conn.fetchrow(
                """SELECT t.id, t.status FROM agent_turns t JOIN agent_sessions s ON s.id = t.session_id
                   WHERE t.id = $1 AND s.owner_id = $2 FOR UPDATE""",
                turn_id,
                owner_id,
            )
            ...
            if existing is None:
                ordinal = await conn.fetchval(
                    "SELECT COALESCE(MAX(ordinal), 0) + 1 FROM agent_steps WHERE turn_id = $1",
                    turn_id,
                )
        ...
        async with self._pool.acquire() as conn, conn.transaction():
            row = await conn.fetchrow(
                """SELECT t.id, t.session_id, t.status FROM agent_turns t
                   JOIN agent_sessions s ON s.id=t.session_id
                   WHERE t.id=$1 AND s.owner_id=$2 FOR UPDATE OF t, s""",
                turn_id,
                owner_id,
            )
            if row is None:
    ...
    async def fail_turn(
        self, owner_id: UUID, turn_id: UUID, code: str, message: str, uncertain: bool
    ) -> None:
    ...
    async def is_auth_session_active(
        self, owner_id: UUID, auth_session_id: UUID
    ) -> bool:
